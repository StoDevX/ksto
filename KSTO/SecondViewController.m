...
- (void)LoadCalendarData
{
    dispatch_sync(kBgQueue, ^{
        ...
        NSString *kGoogleCalendarStr = [NSString stringWithFormat:
                                        @"https://www.googleapis.com/calendar/v3/calendars/kstonarwhal@gmail.com/events?key=%@&orderBy=startTime&timeMin=%@&maxResults=25&singleEvents=true",
                                        key,
                                        dateAndTimeIs];
...
// This is a way to scrol to a specific location in the table view.
-(void) zoomToCurrentShow
{
    // Convert string to date object
    ...
    // Loop through unique date array and find our index
    for (NSString* dateStr in uniqueDateArray)
    {
        if (dateStr == [formatter stringFromDate:date])
        {
            NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:i];
            [tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:i] atScrollPosition:UITableViewScrollPositionTop animated:NO];
        }
        // Increment
        i++;
    }
